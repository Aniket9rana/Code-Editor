...
function showPane(name){
  TAB_ORDER.forEach(k => { 
    if(wraps[k]) {
      // Instead of just .hidden, we manage state for animations
  ...
  requestAnimationFrame(() => {
    ...
    if(ed && ed.resize){ 
      ed.resize(true); 
      // Add a slight fade-in effect to the editor cursor/text
      ed.container.style.opacity = 0;
      ed.focus();
      setTimeout(() => ed.container.style.opacity = 1, 10);
      ed.container.style.transition = 'opacity 0.3s ease';
    }
  });
}